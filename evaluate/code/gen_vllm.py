...
parser.add_argument("--data_path", type=str, default="python")
...
llm = LLM(model=args.model, tensor_parallel_size=torch.cuda.device_count(),gpu_memory_utilization=0.9)
...
for idx, (batch_query, batch_answer, batch_task) in enumerate(
        zip(batch_dataset_query, batch_dataset_answer, batch_dataset_task)):
    with torch.no_grad():
