...
def process_results(doc, completion, answer):
    ...
    if len(split_ans) > 1:
        ...
        # 然后去掉多余的空格，并检查答案是否以句号结尾，如果是，去掉句号。
        ...
        # is_equiv 函数用于判断两个字符串 str1 和 str2 是否等价。
...
def hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    hendrycks_math_ins = []
...
if __name__ == "__main__":
    args = parse_args()
    hendrycks_math(model=args.model, data_path=args.data_file, start=args.start, end=args.end, batch_size=args.batch_size, tensor_parallel_size=args.tensor_parallel_size)
