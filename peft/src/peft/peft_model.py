    ...
    **Attributes**:
    ...
    def save_pretrained(
        ...
        **kwargs: Any,
        ...
        """
            save_directory: 保存模型的目标目录。
            safe_serialization: 是否使用 safetensors 格式保存（默认为 True）。
            selected_adapters: 要保存的适配器名称列表。如果为 None，则默认保存所有适配器。
            save_embedding_layers: 是否保存嵌入层（默认为 auto）。
            is_main_process: 是否是主进程。适用于分布式训练的情况。
            path_initial_model_for_weight_conversion: 用于权重转换的初始模型路径。
            **kwargs: 其他附加参数。
        """
        r"""
        ...
        """
        # 检查 save_directory 是否是一个文件。如果是文件，则抛出异常，因为期望的是一个目录
        if os.path.isfile(save_directory):
            raise ValueError(f"Provided path ({save_directory}) should be a directory, not a file")
        '''
            如果 selected_adapters 为 None，则选择 peft_config 中的所有适配器。
            如果 selected_adapters 提供了适配器名称列表，则检查这些名称是否有效。如果有无效的适配器名称，则抛出异常
        '''
        ...
        '''
            该内部函数用于处理 LoRA 权重转换。
            它首先检查是否使用了不支持的 rank_pattern 或 alpha_pattern。
            如果没有问题，则会尝试将 path_initial_model_for_weight_conversion 中的权重加载并进行相应的转换。
        '''
