...
class CordaConfig:
    ...
    Args:
        cache_file (`Optional[str]`):
            ...
            用于存储SVD缓存的文件。SVD缓存比残差模型小得多（例如，残差
            Llama-3-8b的模型为15GB，而SVD缓存为1.4GB），但使用SVD缓存和原始模型权重，
            可以快速建立残差模型权重。如果你需要重用有限的残差模型权重
            存储，您可以存储SVD缓存。
        covariance_file (`Optional[str]`):
            ...
            存储协方差矩阵的文件。如果你想训练多个不同等级的模型，但它们
            从同一数据集中采样，您可以存储协方差矩阵并将其用于不同的排名。笔记
            协方差文件通常很大（与模型大小相当），因此您需要足够的存储空间
        corda_method (`Literal["ipm", "kpm"]`):
            ...
            构建适配器的方法。KPM（知识保留模式）不仅比LoRA实现了更好的性能
            在微调任务方面，还可以减轻对预先训练的世界知识的灾难性遗忘。什么时候
            保留预先训练好的知识不是问题，IPM（指令预览模式）更受欢迎，因为
            它可以进一步加速收敛，提高微调性能。默认为“ppm”。
        verbose (`bool`):
            If true, prints the progress of CorDA initialization. Defaults to `False`.
            如果为真，则打印CorDA初始化的进度。默认为“False”。
...
class LoraConfig(PeftConfig):
    ...
    init_lora_weights: (
        bool | Literal["gaussian", "eva", "olora", "pissa", "pissa_niter_[number of iters]", "corda", "loftq","tlora"]
    ) = field(
        default=True,
        metadata={
            "help": (
