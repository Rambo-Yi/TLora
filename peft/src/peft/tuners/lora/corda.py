...
def calib_cov_distribution(
    ...
    hooked_model.eval()
    '''
        hook 是一个钩子函数，用来在模型的某些层（模块）进行前向传播时触发，并计算输入数据的协方差矩阵。
        从 input[0] 中提取输入数据，并去掉多余的维度。
        如果不使用 float16 数据类型（由配置决定），则将输入转换为 float。
        输入数据归一化，即按最大绝对值缩放。
        检查输入数据中是否有 NaN 或 Inf，如果有，则抛出错误。
        计算协方差矩阵：input.t().matmul(input)。
        检查计算得到的协方差矩阵是否包含无效值（NaN 或 Inf），如果有，则抛出错误。
        将协方差矩阵累加到模块的 covariance_matrix 中，并增加 sample_count 计数。
        最后释放内存（删除不再需要的变量 covariance 和 input）。
    '''
    ...
    '''
    遍历目标模块（即在 hooked_model 中的模块），为每个模块注册前向钩子 (forward hook)。

    对每个模块，初始化 sample_count 和 covariance_matrix。
    将钩子的句柄 (handle) 保存到 handles 列表中，以便后续删除。

    执行 run_model()，这将触发模型的前向传播并激活钩子。
    '''
    handles = []
    ...
    for handle in handles:
        handle.remove()
    '''
    如果 hooked_model 与 model 不同，意味着我们可能在不同的模型中应用了钩子（例如用于训练一个模型并将协方差矩阵传递给另一个模型）。
    通过将 hooked_model 中模块的 sample_count 和 covariance_matrix 复制到 model 中的相应模块，保持数据同步。
    '''
    ...
    # 对每个模块的协方差矩阵进行归一化，即除以 sample_count，确保计算的协方差矩阵是基于所有样本的平均值
...
'''
这段代码定义了一个名为 collect_eigens_for_layer 的函数，
目的是为给定的线性层（nn.Linear）收集协方差矩阵的特征值分解，
并返回该层的特征值、左奇异向量和右奇异向量。它使用了奇异值分解（SVD）来计算特征值，并进行了一些数值稳定性检查
'''
@torch.no_grad()
