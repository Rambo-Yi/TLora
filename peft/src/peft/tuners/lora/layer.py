...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("tlora"):
           lora_alpha = self.get_base_layer().a
        ...
        elif isinstance(init_lora_weights, str) and init_lora_weights.startswith("tlora"):
            with gather_params_ctx(self.get_base_layer().weight):
                self.tlora_init(adapter_name)
    ...
    def tlora_init(self, adapter_name):
        # 样本X输入构成的激活向量
        linear = self.get_base_layer()
        C = linear.covariance_matrix.to(torch.float32) # [in_features,in_features]
        self.r[adapter_name] = linear.rank
        # # 样本X输入构成的激活向量
        weight = self.get_base_layer().weight.to(torch.float32)
        MixCoV = torch.matmul(weight, C)
        U, S, V = torch.linalg.svd(MixCoV)
        # # 取前 r 个特征向量，这些是数据的主成分
        Vr = V[:self.r[adapter_name], :]  # [r, in_features]
        # # # 构造LoRA矩阵
        self.lora_A[adapter_name] = nn.Linear(self.in_features, linear.rank, bias=False)
        self.lora_B[adapter_name] = nn.Linear(linear.rank, self.out_features, bias=self.lora_bias[adapter_name])

        lora_A  = Vr.contiguous()
        self.lora_A[adapter_name].weight.data = lora_A
        nn.init.zeros_(self.lora_B[adapter_name].weight)
        del linear.covariance_matrix
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        weight = transpose(weight.to(dtype), self.
                           fan_in_fan_out)
